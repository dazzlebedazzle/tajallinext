  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        setProduct(data);
        setSelectedWeight(data.weights?.[0]); // Optional chaining in case weights is undefined
        setSelectedImage(data.images?.[0]);   // Optional chaining in case images is undefined
      } catch (error) {
    ...
    if (slug) fetchProduct(); // Only fetch if slug exists
  }, [slug]);
  ...
  const fetchRelatedProducts = async () => {
    ...
    try {
      const response = await axios.get(`${baseURL}/api/product/allproduct`, {
        params: {
          category: product.category, // ✅ Use category from the product
          limit: 5,
          page
        }
      });

      const currentProductIds = new Set(relatedProducts.map(p => p._id));

      // ✅ Exclude the current product using product._id instead of "title"
      const newProducts = response.data.filter(p => p._id !== product._id && !currentProductIds.has(p._id));
...
}, [product, page]); // ✅ Runs when `product` or `page` changes
      ...
      <div className="product-detail">
        ...
        <div className="product-info">
                    ...
                    <div className="ratings">
                        <span>★★★★★</span> <sup>(120)</sup>
                    </div>
                    <hr />
                    <p className="price my-4"><del className='me-2'>₹{calculatedPrice+100}/-</del>₹{calculatedPrice}/-</p>
